...
**実装時の補足・当初プランからの変更点**:
- `deduction_amounts`・`deduction_items`にはドライバー向けRLSポリシーを追加しない方針を維持しつつ、ホーム画面の前払可能額を算出できるよう、新規マイグレーションに`public.driver_available_advance(p_period_month date)`（SECURITY DEFINER、`acknowledge_payment_notice`と同様に引数のdriver_idを信用せず`current_driver_id()`から導出）を追加した。プラン記載時点では未検討だった穴（RLSを追加しない範囲のデータをどう安全に集計するか）を埋める形。
- `lib/apiAuth.ts`の`requireDriver(req, res)`は`profiles.role==='ドライバー'`かつ`drivers.profile_id`で紐づく行の存在まで確認し、`{ supabase, profile, driver }`を返す。
- `types/domain/mypage.ts`を新規追加（`MySummary`・`MyOrderRow`・`MyOrderMonth`・`MyPaymentRow`・`MyPaymentDetail`）。
- 現状、マスタ管理のドライバー作成フォーム（`POST /api/master/drivers`）・設定画面のアカウント作成（`POST /api/settings/accounts`）のいずれも「ドライバーのログインアカウントを既存の`drivers`行に紐づける」操作に対応していない。動作確認時はテスト用に直接SQLで`profiles.role`と`drivers.profile_id`を設定した。今後、実運用でドライバーにログインアカウントを発行する導線（既存ドライバーへのアカウント紐付けUI）が別途必要になる可能性が高い。
- ブラウザ検証（Playwright）で、ホーム（稼働日数・前払可能額の実数値表示）、発注書一覧＋月間稼働カレンダーモーダル、支払通知書一覧（仮確定／確定タブ）、詳細モーダルでの件数内訳表示、「確認しました」ボタン（個別、仮確定・確定どちらの状態でも動作）を確認済み。確認後は`supabase db reset`でテストデータをクリア。
...
## 追加機能: CSVインポート・ショートカット（件数集計／管理費集計／発注書／支払通知書／前払依頼書）

`Space+c→Space+s→Space+v`の順次キー押下（入力欄フォーカス中は無効）でCSVインポートモーダルを開く機能。ボタンは併設せずショートカットのみとした（ユーザー確認済み）。

- `lib/useCsvImportShortcut.ts` — キー列検出の共通フック（1.5秒以内に3ストローク）
- `components/CsvImportModal.tsx` — ファイル選択＋取り込み結果表示の共通モーダル
- 件数集計・管理費集計・発注書：既存のCSVエクスポート列構成をそのままインポートにも使用し、既存の保存API（`/api/counts`・`/api/cost/amounts`・`/api/schedule/days`）に流し込む。件数集計は日次表示中のみ有効。
- 前払依頼書：`ドライバー・入金日・前払金額・備考`列から`/api/advance`で新規依頼を一括作成（依頼No・前払可能額・ステータスは自動算出のため対象外）
- 支払通知書：**テストデータ投入用**と明示のうえ、新規`POST /api/payments/import`で`payment_notices`を直接upsert（既存の生成→承認→確定フロー、確定後の再承認ルールを経由しない）。ユーザー確認済みの割り切った仕様。
