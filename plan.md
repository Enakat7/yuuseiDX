...
- Phase 2から蓄積されている`operation_logs`を一覧表示。管理者限定（クライアント側ガード＋RLSの二重）。
- **要決定事項**: ブラウザにSupabaseセッションがないため、標準のRealtime購読（`supabase.channel()`）がそのままでは認証されない。着手時に以下のいずれかを選ぶ:
  - (a) サーバー側で短命トークンを発行し、ブラウザの`supabase.realtime.setAuth(token)`に渡してから購読する
  - (b) Realtimeを使わず、`GET /api/logs?since=`を数秒間隔でポーリングする（実装は単純だが「リアルタイム」の要件強度がやや落ちる）
  - 着手時にどちらを採るか判断する。
